...
-- Task to calculate hourly F&G index every hour
CREATE OR REPLACE TASK ANALYTICS.TASK_CALCULATE_HOURLY_FNG
    WAREHOUSE = COMPUTE_WH
    SCHEDULE = 'USING CRON 5 * * * * UTC'  -- 5 minutes past every hour
...
CREATE OR REPLACE TASK ANALYTICS.TASK_CALCULATE_DAILY_FNG
    ...
    AFTER ANALYTICS.TASK_CALCULATE_HOURLY_FNG
AS
...
-- Uncomment these to enable automated processing:
